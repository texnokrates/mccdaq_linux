...
usb26xx_model = {2623: USB2623_PID, 2627: USB2627_PID, 2633: USB2633_PID, 2637: USB2637_PID}

cdef class USB2600_Wrapper:
  ...
  def __cinit__(self, model=2627):
    """
    :param model: USB-26xx model (2623, 2627, 2633 or 2637)
    :type  model: int"""
    self.usb_handle=usb_device_find_USB_MCC(usb26xx_model[model])
    usbInit_2600(self.usb_handle)
    ...
    cdef np.ndarray[np.float32_t, mode="c",ndim=2] tAI = \
      np.ascontiguousarray(np.empty(shape=(NGAINS_2600,2),dtype=np.float32))
    cdef np.ndarray[np.float32_t, mode="c",ndim=2] tAO = \
      np.ascontiguousarray(np.empty(shape=(NCHAN_AO_26X7,2),dtype=np.float32))
    usbBuildGainTable_USB2600(self.usb_handle, &tAI[0,0]);
    usbBuildGainTable_USB26X7(self.usb_handle, &tAO[0,0]);
    self.table_AI = tAI
    self.table_AO = tAO    
  ...
  def print_calibration_table(self):
    ...
    cdef np.ndarray[np.float32_t, mode="c",ndim=2] tAO = \
      np.ascontiguousarray(self.table_AO, dtype=np.float32)
    print("\nAnalog Input Calibration Table\n");
    for i in range(NGAINS_2600):
  ...
  def __str__(self):
    return "<MCC USB-2627 (low-level interface)>"
  
  def __del__(self):
    print("Closing device.")
    cleanup_USB2600(self.usb_handle)
    freeScanList(self.scanList)
    
  cdef float AItoVolts(self, voltage):
    gain = 0
    return volts_USB2600(self.usb_handle, gain, 
  ...
  def AIn(self, channel):
    ...
    usbAInConfig_USB2600(self.usb_handle, self.scanList)
    
    return self.AItoVolts(usbAIn_USB2600(self.usb_handle, channel))
  ...
  def AInScanStart(self, channels, nsamples, rate,
    ...
    usbAInScanStart_USB2600(self.usb_handle, nsamples*retrigger, nsamples, rate, packet_size, options);
  ...
  def AInScanRead(self, nchannels, nsamples):
    ...
    bytes_read = usbAInScanRead_USB2600(self.usb_handle, nsamples, nchannels, &data[0])
    if bytes_read < 0:
  ...
  def AInScanStop(self):
    """Stop the AI scan. """ 
    usbAInScanStop_USB2600(self.usb_handle)
    
  def AInScanClear(self):
    """ Clear the AI FIFO. Data will be lost! """
    usbAInScanClearFIFO_USB2600(self.usb_handle)
  ...
  def TriggerConfig(self, action='Set', mode='Edge', polarity='Rising'):
    ...
    cdef int options
    if action == 'Set':
      usbTriggerConfig_USB2600(self.usb_handle, self.trigger_mode[mode] + (self.trigger_polarity[polarity] << 1))
    else:
      ...
      usbTriggerConfigR_USB2600(self.usb_handle, &options)
  ...
  def blink(self, count=1):
    cdef int c = count
    usbBlink_USB2600(self.usb_handle, c)
    
  def temperature(self):
    cdef float temp
    usbTemperature_USB2600(self.usb_handle, &temp)
    return temp
  
  def serialNumber(self):
    cdef char *ser
    ser = "123465789"
    usbGetSerialNumber_USB2600(self.usb_handle, ser)
    return ser
  
  def status(self):
    """Returns the device status byte."""
    status_byte = usbStatus_USB2600(self.usb_handle)
    #print("Status = %#x\n", status_byte)
  ...
  def FPGAVersion(self):
    cdef int version
    usbFPGAVersion_USB2600(self.usb_handle, &version)
    return version
 
  def reset(self):
    """Causes the device to perform a reset, disconnects from the USB bus and resets its microcontroller."""
    usbReset_USB2600(self.usb_handle)
  
  def counterInit(self, counter):
    usbCounterInit_USB2600(self.usb_handle, counter)
    
  def counterRead(self, counter):
    return usbCounter_USB2600(self.usb_handle, counter)
  ...
  def dioConfig(self, port, value=None):
    ...
    """
    if value is None:
      return usbDTristateR_USB2600(self.usb_handle, self.port[port]) 
    else:     
      usbDTristateW_USB2600(self.usb_handle, self.port[port], value)
    
  def dioWrite(self, port, value):
    usbDLatchW_USB2600(self.usb_handle, self.port[port], value)
  
  def dioRead(self, port):
    return usbDLatchR_USB2600(self.usb_handle, self.port[port])
  
  def timerControlRead(self, timer):
    cdef int control = 0
    usbTimerControlR_USB2600(self.usb_handle, timer, &control)
  ...
  def timerControlWrite(self, timer, enable, invert=False):
    ...
    usbTimerControlW_USB2600(self.usb_handle, timer, control)

  def timerPeriodWrite(self, timer, period):
    """ frequency = 64 MHz / (period + 1) """
    usbTimerPeriodW_USB2600(self.usb_handle, timer, period)
  
  def timerPeriodRead(self, timer):
    cdef int period = 0
    usbTimerPeriodR_USB2600(self.usb_handle, timer, &period)
    return period
  
  def timerPulseWidthWrite(self, timer, pulse_width):
    usbTimerPulseWidthW_USB2600(self.usb_handle, timer, pulse_width)
  
  def timerPulseWidthRead(self, timer):
    cdef int pw = 0
    usbTimerPulseWidthR_USB2600(self.usb_handle, timer, &pw)
    return pw
  
  def timerCountRead(self, timer):
    cdef int count = 0
    usbTimerCountR_USB2600(self.usb_handle, timer, &count)
    return count
  
  def timerCountWrite(self, timer, count):
    usbTimerCountW_USB2600(self.usb_handle, timer, count)
    
  def timerDelayRead(self, timer):
    cdef int delay = 0
    usbTimerDelayR_USB2600(self.usb_handle, timer, &delay)
    return delay
  
  def timerDelayWrite(self, timer, delay):
    usbTimerDelayW_USB2600(self.usb_handle, timer, delay)    
